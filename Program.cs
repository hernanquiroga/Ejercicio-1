...
var largoArray = Console.ReadLine();  // Ingreso el tamaño del vector
int n = Convert.ToInt32(largoArray); // Convierto a entero   

int[] miVector = new int[n];    // Declaro el vector

Random r = new Random();    // Genero un numero al azar

for (int i = 0; i < miVector.Length; i++) // Guardo los valores generados por Random(menores a 100) en el array.
{
    miVector[i] = r.Next(100);
}


foreach (int item in miVector) { // Muestro los valores del vector
...
//el anteúltimo en el segundo y así sucesivamente.
